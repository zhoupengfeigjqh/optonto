...
（概念基本信息）
- RawMaterial（原材料）：表示生产所需的物料，具有编号、名称、单位、安全库存等属性。
- Supplier（供应商）：提供原材料的组织，具有编号、名称、地址、联系人、联系电话等属性。
- PurchaseRecord（原材料采购记录）：记录一次原材料采购的详细信息，包括采购单号、原材料、数量、时间、供应商、关联订单等。
- RawMaterialInventory（原材料库存）：记录某种原材料的库存快照，包括待入库量、累计库存量、可用库存等。
- CustomerOrder（客户订单）：表示客户下达的生产订单，具有订单编号、名称、批次、缓冲期、类型、状态等属性。
- DeliveryCapability（交货能力）：表示供应商对某种原材料的供货能力，包括交货周期、最小起订量等。

（概念关系基本信息）
- hasRawInventory（拥有库存）：RawMaterial -> RawMaterialInventory (1:1)，一种原材料对应一个库存记录。
- recordsRawMaterial（记录原材料）：PurchaseRecord -> RawMaterial (N:1)，多条采购记录可以对应同一种原材料。
- orderedFromSupplier（从供应商采购）：PurchaseRecord -> Supplier (N:1)，多条采购记录可以对应同一个供应商。
- suppliesRawMaterial（供应原材料）：Supplier -> RawMaterial (N:M)，一个供应商可以供应多种原材料，一种原材料可由多个供应商供应。
- containsPurchaseRecord（包含采购记录）：CustomerOrder -> PurchaseRecord (1:N)，一个客户订单可以包含多条采购记录。
- hasCapability（具有能力）：Supplier -> DeliveryCapability (1:N)，一个供应商可以有多个供货能力记录。
...
（原材料库存和采购的核心业务流程）
1. 用户或系统查询原材料库存（QueryInventory），判断是否需要采购。
2. 根据库存情况或客户订单需求，创建原材料采购单（CreatePurchaseRecord）。
3. 定期或主动按需查询采购记录（QueryPurchaseRecords），跟踪已创建的采购单状态。
4. 确认原材料到货后，执行入库操作（ReceiveRawMaterial），更新库存。
5. 入库完成后，再次查询库存（QueryInventory），确认库存状态已更新。
...
- 调用工具：executeOntoBehavior，参数为行为名称CreatePurchaseRecord及上述输入参数。
...
- 调用工具：executeOntoBehavior，参数为行为名称CancelPurchaseRecord及上述输入参数。
...
- 调用工具：executeOntoBehavior，参数为行为名称ReceiveRawMaterial及上述输入参数。
...
- 输出结构：
  ...
  ```
- 相关概念：RawMaterialInventory（原材料库存）、RawMaterial（原材料）
- 调用工具：executeOntoBehavior，参数为行为名称QueryInventory及上述输入参数。
...
- 输出结构：
  ...
  ```
- 相关概念：PurchaseRecord（原材料采购记录）
- 调用工具：executeOntoBehavior，参数为行为名称QueryPurchaseRecords及上述输入参数。
...
- 调用工具：executeOntoBehavior，参数为行为名称QueryRawMaterials及上述输入参数。
...
- 输出结构：
  ...
  ```
- 相关概念：Supplier（供应商）
- 调用工具：executeOntoBehavior，参数为行为名称QuerySuppliers及上述输入参数。
...
- 输出结构：
  ...
  ```
- 相关概念：DeliveryCapability（交货能力）、Supplier（供应商）
- 调用工具：executeOntoBehavior，参数为行为名称QuerySupplierCapability及上述输入参数。
...
- 输出结构：
  ...
  ```
- 相关概念：DeliveryCapability（交货能力）、Supplier（供应商）
- 调用工具：executeOntoBehavior，参数为行为名称QuerySupplierCapabilitySQL及上述输入参数。
...
- 返回结构：
  ...
  ```
- 调用工具：executeOntoFunction，参数为函数名称sumRawNotArrivalQty及上述输入参数。
