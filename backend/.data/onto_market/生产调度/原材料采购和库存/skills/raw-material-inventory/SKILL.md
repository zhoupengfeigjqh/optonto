...
原则3：流程执行原则，用户输入-意图分析-本体抽取（skill的内容）-总任务生成-子任务拆分-子任务循环按顺序执行（子任务信息-LLM-选择行为-前置规则-前置安全管控-调用工具（执行行为）-结果（工具输出）-后置规则-后置安全管控）-下一个子任务执行-...-结束，流程中有错误或者规则不满足则反馈，若触发安全管控则反馈需要人工确认！】
