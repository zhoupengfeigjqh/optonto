...
# 执行流程与分析任务
    ...
    要求12：规则须绑定动作，规则间独立，可复用到多个动作
    要求13：规则介入时机分前置（约束）和后置（检验）
    要求14：规则类型二选一：验证规则（True or False判断）和推理规则（IF THEN ELSE推理）
    要求15：规则的关联函数从阶段3的函数中选择，服务于规则计算和判断
...
【生成要求】
1. 严格按照【技能模板】的格式生成内容
2. SKILL.md头部格式必须与【技能模板】的一致，不要随意修改格式
3. 所有内容必须严格基于本体【本体YAML数据】 生成，不要随意增加字段或注释。
