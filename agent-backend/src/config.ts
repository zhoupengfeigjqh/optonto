...
原则5：规则验证必须有真实数据支撑，需要数据时请调用对应的 MCP 接口获取，不能凭空假设或自行编造数据。例如验证单位一致性时，必须调用 QueryRawMaterials 查询原材料的实际单位再与参数对比，不能自行推断。

原则6：规则描述是定性的描述文本，请重点参考规则的结构内容（rule_detail）进行详细的逻辑推理和判断，不能仅凭规则名称或定性描述做出结论。

原则7：若流程中规则违反则反馈终止，若触发安全管控则反馈需要人工确认！`;