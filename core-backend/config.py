...
# 执行流程与分析任务
    ...
    - **子任务**：
        ...
        - **子任务2-行为**：需提供英文名、中文名、类型、描述、关联概念、输入参数和返回结构。
    ...
    要求7：行为的类型有SQL和API类型两种，两种方式决定了后续不同的数据映射方式（前者是db schema映射，后者是api接口映射），请务必和用户讨论确认方式；
    要求8：行为须关联至少一个概念，可关联多个；
    要求9：行为输入参数采用JSON格式，字段须来自概念属性，其参考格式如下：
    {
        "rawMaterialId": {
            "type": "string",
            "required": true,
    ...
    要求10：行为返回结构采用JSON格式，字段须来自概念属性，其参考格式如下：
    {
    "code": {
        "type": "number",
        "example": 200
    ...
    要求11：请用户提供目标数据源接口信息以便设计，若用户无法提供则自行合理设计
    要求12：规则须绑定一个动作，规则间独立，可复用到多个动作
    要求13：规则可关联多个函数，其从阶段3的函数中选择，服务于规则计算和判断
    要求14：规则的数据补充，是指规则进行判断时，需要额外拉取其他概念属性的数据，请根据概念属性，选择能获得该数据的查询行为，可以选多个
    要求15：规则介入时机分前置（约束）和后置（检验）
    要求16：规则类型二选一：验证规则（True or False判断）和推理规则（IF THEN ELSE推理）
...
4. 行为和函数的【输入参数】与【输出参数】必须提供**完整结构**，以json格式展示。任何行为/函数都不得省略输入或输出参数说明，不得漏标必填字段。其中executeOntoBehavior调用行为执行，executeOntoFunction调用函数
