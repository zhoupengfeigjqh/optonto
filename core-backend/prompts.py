...
# 执行流程与分析任务
    ...
    ## 阶段7：全局检查与汇总
    - **任务**：汇总阶段1至阶段6的全部输出，注意必须原封不动汇总，不能省略明细。
...
4. 行为和函数的【输入参数】与【输出参数】必须提供**完整结构**，以json格式展示。任何行为/函数都不得省略输入或输出参数说明，不得漏标必填字段。其中executeOntoBehavior调用行为执行，本体函数与公共函数均作为 MCP 工具直接调用
