...
import { getRules, createRule, updateRule, deleteRule, getBehaviors, getFunctions, getRuleTemplateTypes, Rule, Behavior, Function } from '@/api/client';
...
export default function RuleTable({ ontologyId, activeTab }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [ruleList, behList, fnList, types] = await Promise.all([getRules(ontologyId), getBehaviors(ontologyId), getFunctions(ontologyId), getRuleTemplateTypes()]);
